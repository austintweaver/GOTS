        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-center space-x-6">
            <Label 
              ...
              className={`text-lg font-semibold transition-colors ${showBrands ? 'text-brand-red' : 'text-gray-500'}`}
            >
              For Brands
            </Label>
            <Switch
              id="service-toggle"
              checked={!showBrands}
              onCheckedChange={(checked) => setShowBrands(!checked)}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {brandServices.map((service) => (
                  <Card 
                    key={service.title}
                    className="group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border-gray-200 hover:border-brand-red/20"
                  >
            ...
            <div className="bg-gray-50 rounded-3xl p-8 md:p-12">
                ...
                {gamePillars.map((pillar, index) => (
                  ...
                  >
                    <div className="relative mb-6">
                      ...
                      {index < gamePillars.length - 1 && (
                        <div className="hidden lg:block absolute top-10 left-full w-full h-0.5 bg-gradient-to-r from-brand-red/30 to-transparent transform -translate-y-1/2"></div>
                      )}
                    </div>
              ...
              <div className="text-center space-y-4 md:space-y-0 md:space-x-4 md:flex md:justify-center">
                <Button 
                  ...
                  asChild
                >
                  <Link to="/contact">Join The GAME Plan</Link>
                </Button>
                <Button 
                  size="lg"
                  variant="outline"
                  className="border-brand-red text-brand-red hover:bg-brand-red hover:text-white font-semibold px-8 py-4 text-lg transition-all duration-300"
                  asChild
                >
